...
def inference_multiview(views, n_classes, keep_prob):
    """
    views: N x V x S x W x H tensor
    """
    n_views = views.get_shape().as_list()[1] 
    n_scals = views.get_shape().as_list()[2]
    # transpose views : (NxVxSxWxH) -> (VxSxNxWxH)
    views = tf.transpose(views, perm=[1, 2, 0, 3, 4])
    
    view_pool = []
    for i in range(n_views):
        for j in range(n_scals):
        # set reuse True for i > 0, for weight-sharing
            reuse = (i != 0 or j!=0)
            viewi = tf.gather(views, i) # NxWxHxC
            viewj = tf.gather(viewi, j)
            view= tf.expand_dims(viewj,axis=-1)
    
            conv1 = _conv('conv1', view, [5, 5, 1, 96], [1, 1, 1, 1], 'VALID', reuse=reuse)
    #        lrn1 = None
#            pool1 = _maxpool('pool1', conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
    
            conv2 = _conv('conv2', conv1, [5, 5, 96, 256], [1, 1, 1, 1], 'VALID', reuse=reuse)
    #        lrn2 = None
#            pool2 = _maxpool('pool2', conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
            
            conv3 = _conv('conv3', conv2, [3, 3, 256, 256], [1, 1, 1, 1], 'VALID', reuse=reuse)
            conv4 = _conv('conv4', conv3, [3, 3, 256, 128], [1, 1, 1, 1], 'VALID', reuse=reuse)
            conv5 = _conv('conv5', conv4, [3, 3, 128, 96], [1, 1, 1, 1], 'VALID', reuse=reuse)
    
    #        pool5 = _maxpool('pool5', conv5, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
            
            dim = np.prod(conv5.get_shape().as_list()[1:])
            reshape = tf.reshape(conv5, [-1, dim])
    ...
    fc6 = _fc('fc6', pool5_vp, 1024, dropout=keep_prob)
    fc7 = _fc('fc7', fc6, 512, dropout=keep_prob)
