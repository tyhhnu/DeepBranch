...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 19 15:07:54 2018
...
def getcand(imgvox,marker,swc, margin,n_each):
#    ones = np.argwhere(labelmap>0)
    zeros = np.argwhere(imgvox>0)
    np.random.shuffle(swc)
    np.random.shuffle(zeros)
    idx1 = zeros[0:n_each//2,:]
    idx2 = swc[0:n_each//2,:]
#    idx = idx[0:n_each,:]
    idx = np.vstack((idx2,idx1))
    return idx+margin
...
if __name__ == '__main__':
    ...
    img_name = 'm3.tif'
    n_each = 1000
    idx = getcand(imgvox,marker, swc, margin,n_each)
