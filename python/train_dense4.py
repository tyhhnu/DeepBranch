...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 11 10:50:30 2018
...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 27 15:47:30 2018
...
tf.app.flags.DEFINE_string('train_dir', '/media/ttt/Elements/TanYinghui/train_dense4t2.hdf5',
                           """h5file of trian data """
                           """and checkpoint.""")
tf.app.flags.DEFINE_boolean('log_device_placement', False,
                            """Whether to log device placement.""")
tf.app.flags.DEFINE_integer('block_depth', 25,
...
def train(trainpath,  ckptfile=''):
    ...
    with tf.Graph().as_default():
        ...
        y_ = tf.placeholder('int64', shape=(None,V,V,VZ), name='y')
...
#            print ('loaded pretrained caffemodel:', caffemodel)
        ...
        for epoch in range(FLAGS.epoch):
            ...
            for i, idx in enumerate(trainidx):
                ...
                x, y,  = select_patches_from(imfile,idx)
                num, nrotate, nscale, kernelsz, _, _ = x.shape
#                num =len(x)
                ...
                while batch_start < num:
                    ...
                    if batch_end <= num :
                        ...
                        batch_x = np.zeros((nsample_each,  V, V, VZ))
#                        xuhao = np.arange(batch_x.shape[0])
#                        np.random.shuffle(xuhao)
                        batch_y = np.zeros((nsample_each, V,V,VZ))
    #                    coords = np.zeros((nsample_each, 3))             
                        batch_x[:, :, :, :, ] = x[batch_start:batch_end,0,0,:,:,:]
                        batch_x = np.expand_dims(batch_x,-1)
#                        batch_x = batch_x.transpose((0,4,2,3,1))
                        batch_y =y[batch_start:batch_end,0,0,:,:,:]
...
#                        print('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
                        if step % 100 == 0 and step > startstep and step >3000:
#                            print('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        #                    checkpoint_path = os.path.join(FLAGS.train_dir, 'model.ckpt')
                            saver.save(sess, '/home/ttt/model5/model5.cpkt', global_step=step)
